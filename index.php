    ...
    if($_SERVER["REQUEST_METHOD"] == "PUT"){
        ...
        if(empty($usuario_err) && empty($senha_err)){
            ...
            if($stmt = mysqli_prepare($link, $sql)){
                ...
                mysqli_stmt_bind_param($stmt, "ssssss", $param_usuario, $param_senha, $param_salt);
                
                // Set parameters
                $param_usuario = $usuario;
                $param_salt = password_hash(random_bytes(90), 'sha1');
                $param_senha = password_hash($senha + $param_salt, 'sha256');
                
                // Attempt to execute the prepared statement
                if(mysqli_stmt_execute($stmt)){
                    // Records created successfully. Redirect to landing page
                    header("location: TO_DEFINE.php");
                    exit();
                } else{
            ...
            if($stmt = mysqli_prepare($link, $sql)){
                ...
                if(mysqli_stmt_execute($stmt)){
                    ...
                    if(mysqli_num_rows($result) == 1){
                        ...
                        if(password_verify($senha + $saltBanco, $senhaBanco)){
                            header("location: TO_DEFINE.php");
                            exit();
                        }
                    } else{
				...
				<!-- Main -->
                    ...
                    <!-- Login -->
                        <article id="login">
                            ...
                            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="put">
                                <div class="fields">
